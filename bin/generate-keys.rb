...
class Extractor
  attr_accessor :name, :keys, :package, :is_auto_plugin

  def initialize(lines)
    @name = /object (\w+) (extends (Auto)?Plugin)*/.match(lines)[1]
    @is_plugin = /object \w+ extends (Auto)?Plugin /.match(lines)
    @is_auto_plugin = /object \w+ extends AutoPlugin /.match(lines)
...
PLUGINS_TO_S
    auto = ex.is_auto_plugin ? '.autoImport' : ''
    ex.keys.each do |key|
      s = s + "val #{key} = #{ex.name}#{auto}.#{key}\n"
    end
    "#{s}\n"
  end  
end
...
class ScalaFileHandler
  ...
  def is_eligible?
    @lines.grep(/object \w+ extends (Auto)?Plugin /).length > 0 || @lines.grep(/object Keys \{/).length > 0
  end
